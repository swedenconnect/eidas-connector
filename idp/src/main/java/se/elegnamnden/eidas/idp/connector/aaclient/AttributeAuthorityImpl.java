...
public class AttributeAuthorityImpl implements AttributeAuthority, InitializingBean {
  ...
  public List<Attribute> resolveAttributes(List<Attribute> attributes, String country) throws AttributeAuthorityException {
    ...
    PridResponse pridResponse = this.pridService.getPrid(AttributeUtils.getAttributeStringValue(eidasPersonIdentifier), country);
    ...
    resolvedAttributes.add(AttributeConstants.ATTRIBUTE_TEMPLATE_PRID_PERSISTENCE.createBuilder().value(pridResponse.getPidQuality()).build());
    ...
    if (pridResponse.getPersonalIdNumber() != null) {
      resolvedAttributes.add(AttributeConstants.ATTRIBUTE_TEMPLATE_PERSONAL_IDENTITY_NUMBER.createBuilder()
        .value(pridResponse.getPersonalIdNumber()).build());
      ...
      if (pridResponse.getPersonalIdNumberBinding() != null) {
        resolvedAttributes.add(AttributeConstants.ATTRIBUTE_TEMPLATE_PERSONAL_IDENTITY_NUMBER_BINDING.createBuilder()
          .value(pridResponse.getPersonalIdNumberBinding()).build());
      }
      else {
        log.warn("PRID service returned personal identity number, but no corresponding binding URI - using '{}'", UNSPECIFIED_PERSONAL_IDENTITY_NUMBER_BINDING);
        resolvedAttributes.add(AttributeConstants.ATTRIBUTE_TEMPLATE_PERSONAL_IDENTITY_NUMBER_BINDING.createBuilder()
          .value(UNSPECIFIED_PERSONAL_IDENTITY_NUMBER_BINDING).build());
      }
    }
