...
docker run -d --name eidas-connector-dev-idp --restart=always \
  ...
  -e EIDAS_METADATA_SERVICE_LIST_URL=https://mdsl.sandbox.swedenconnect.se/nodeconfig/mdservicelist \
  -e EIDAS_METADATA_SERVICE_LIST_VALIDATION_CERT=/opt/eidas-connector/shibboleth/metadata/eidas-servicelist-validation-cert.crt \
